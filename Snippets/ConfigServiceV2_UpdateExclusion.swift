...
func sample(client: ConfigServiceV2Client, projectId: String, exclusionId: String) async throws {
  let response = try await client.updateExclusion(
    request: UpdateExclusionRequest()
      .with {
        $0.exclusion = LogExclusion().with {
          $0.name = "projects/\(projectId)/exclusions/\(exclusionId)"
        }
...
struct SnippetRunner {
  static func main() async throws {
    do {
      ...
      try await sample(client: client, projectId: "[placeholder]", exclusionId: "[placeholder]")
    } catch {
      print("Error: \(error)")
    }
  }
